...
call spInsertProduto(12345678910111,'Rei de Papel Mache',65.00,120);
...
CALL spUpdateProduto(12345678910111, "Rei de Papel Mache", 65.00);
...
call spInsertVenda("Lança Perfume",12345678910114,15);
select * from tbVenda;
select * from tbItemVenda;
select * from tbProduto;
select * from tbCliente;
...
call spInsertVenda("Paganada",12345678910114,10);
...
select tbCliente.IdCli, tbCliente.NomeCli, tbClientepj.CNPJ, tbClientepj.IE,tbClientepj.IDCli
from tbCliente inner join tbClientepj
...
-- Exercício 36
select 
tbCliente.IdCli, tbCLiente.NomeCli,tbCliente.NumEnd,tbCLiente.CompEnd,tbCLiente.CepCli,tbCLientepj.CNPJ,tbCLientepj.IE,tbClientepj.IdCli,tbEndereco.Logradouro,tbEndereco.IdBairro,tbEndereco.IdCidade,tbEndereco.IdUF,tbEndereco.CEP
from tbCliente inner join tbClientepj inner join tbEndereco
on tbEndereco.CEP = tbCliente.CepCli and tbCliente.IdCli = tbClientepj.IdCli;

-- Exercício 37
select 
tbCliente.IdCli, tbCLiente.NomeCli,tbEndereco.CEP, tbEndereco.Logradouro,tbCliente.NumEnd,tbCLiente.CompEnd,tbBairro.Bairro,tbCidade.Cidade,tbUF.UF 
from tbCliente inner join TbClientepj inner join tbEndereco inner join tbCidade inner join tbBairro inner join tbUF 
on tbEndereco.CEP = tbCliente.CepCli and tbCLiente.IdCli = tbClientepj.IdCli and tbEndereco.IdBairro = tbBairro.IdBairro and tbUF.IdUF = tbEndereco.IdUF and tbCidade.IdCidade = tbEndereco.IdCidade  where CNPJ is not null;
select * from tbClientepj;

-- Exercício 38

DELIMITER $$
create procedure spConsultaClienteId (vIdCli int) begin
select 
tbCliente.IdCli as 'Código', 
tbCliente.NomeCli as 'Nome', 
tbClientepf.CPF, 
...
tbCidade.Cidade,
tbUf.Uf 
from tbCliente inner join tbEndereco inner join tbBairro inner join tbCidade inner join tbUF inner join tbClientePF
on tbClientepf.IdCli = tbCliente.IdCli and tbCliente.CEPCli = tbEndereco.CEP and tbEndereco.IdCidade = tbCidade.IdCidade and tbBairro.IdBairro = tbEndereco.IdBairro and tbUf.IdUf = tbEndereco.IdUF 
where tbCLiente.IdCli = vIdCli;
end 

$$ 
select * from tbCLiente;
call spConsultaClienteId(9);
...
select tbFornecedor.IdFornecedor, tbFornecedor.CNPJ, tbFornecedor.NomeFornecedor, tbFornecedor.telefone from tbCompra
right join tbFornecedor 
on tbCompra.IdFornecedor = tbFornecedor.IDFornecedor where CodigoCompra is null;

-- Exercício 42

select tbCliente.IdCLi, tbCLiente.NomeCli, tbVenda.DataVenda, tbProduto.CodigoBarras, tbProduto.NomeProd, tbItemVenda.ValorItem from tbCliente 
inner join tbVenda inner join tbProduto inner join tbItemVenda
on tbCliente.IdCli = tbVenda.IdCli and tbProduto.CodigoBarras = tbItemVenda.CodigoBarras and tbVenda.CodigoVenda = tbItemVenda.CodigoVenda where tbItemVenda.CodigoVenda is not null order by NomeCli asc;
